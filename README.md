...
- `/` — the English article, with a code-drawn 2D balance cover and locally served mathematics.
...
The article cover is hand-coded SVG in `assets/balance-diagram.svg`, matching the game’s flat silver balance and numbered coins. A gentle six-second CSS animation rocks the beam while keeping the pans upright; reduced-motion preferences disable it. The original generated photo remains in `assets/balance-cover.png` for social previews; its prompt is preserved in `assets/cover-prompt.md`.
